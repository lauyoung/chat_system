...
void exe_cgi(int sock_client, const char *path, const char *method,const char *query_string)
{
	...
	{//POST方法，web服务器通过stdin向cgi程序提交数据，数据的最后并没有EOF
	 //标志，因此为了正确提交数据，必须有content_length.
		do{
			memset(buf, '\0', sizeof(buf));
	...
	if((id = fork()) < 0){//fork error
		...
		close(cgi_output[0]);
		//redir
		dup2(cgi_input[0], 0);//标准输入重定向
		dup2(cgi_output[1], 1);//标注输出重定向
